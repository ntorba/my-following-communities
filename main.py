...
username = st.text_input("Enter a username", value="nicktorba")
...
if "errors" in data:
    ...
    if not os.path.exists(f"data/{username}_following.csv"):
        ...
        with st.spinner("Loading community info from borg"):
    ...
    username = st.selectbox("Select a username to see which communities they belong to", min_group["username"].unique())
    # get the communities that user belongs in
    user_communities = borg_community_df[borg_community_df["username"] == username]["clusters.name"].unique()
